const DEFAULT_API_URL = 'https://deadsmile.vercel.app/api';
...
function validatePath(path) {
  if (typeof path !== 'string' || !path.startsWith('/') || path.startsWith('//')) {
    throw new ApiError('Invalid API request path.', 0, 'INVALID_REQUEST_PATH');
  }
}
...
async function fetchWithTimeout(url, options) {
  ...
  try {
    ...
    if (error?.name === 'AbortError') {
      throw new ApiError('The server took too long to respond. Please try again.', 0, 'REQUEST_TIMEOUT');
    }
