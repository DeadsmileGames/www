      ...
      <div className="container game-details__body">
        ...
        <aside className="game-details__aside">
          ...
          <div className="game-details__trailer-list">
            {trailerVideos.map((video, index) => <Link to={`/?video=${encodeURIComponent(video.id)}#videos`} className="btn btn--primary game-details__trailer" key={video.id}><Play weight="bold" />{trailerVideos.length > 1 ? `Watch trailer ${index + 1}` : 'Watch trailer'}</Link>)}
